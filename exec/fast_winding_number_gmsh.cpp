...
struct OrientedEdge {
    ...
    OrientedEdge(Point2<Scalar> _s, Point2<Scalar> _t) : s(_s), t(_t) {}
...
float fast_wn(const Point2<double>& q, const Node& node, const Tree& tree){

    Point2<double> vec = node.data.p - q;
    float norm = vec.norm();
    if (norm > beta*node.data.r) {
        ...
        if (node.isleaf()){
            for (size_t i = 0; i < node.obj.size(); i++) {
                ...
                Point2<double> dir = edge.t - edge.s;
                Point2<double> n(dir[1], -dir[0]);

                Point2<double> pi = edge.get_centroid();
                Point2<double> d = pi - q;
                float dist = d.norm();
...
int main(int argc, char** argv){
    ...
    for (size_t i = 0; i < bnd_edges.size()/2; i++){
        size_t tag_s = bnd_edges[2*i];
        size_t tag_t = bnd_edges[2*i+1];
        Point2<double> s( nodeCoords[3*tag_s], nodeCoords[3*tag_s+1]);
        Point2<double> t( nodeCoords[3*tag_t], nodeCoords[3*tag_t+1]);
    ...
    std::vector<Point2<double>> ptest(w*h);
    std::vector<double> wn(w*h);
